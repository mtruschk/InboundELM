...
def extract_information(xml_path, base_name):
    ...
    for user_object in root.findall('.//UserObject') + root.findall('.//object'):
        shapetype = user_object.get('btelligentShapeType')
    ...
    if len(connections) > 0:
        ...
        SELECT c.base_name
             ...
             , s.shapetype                                                                  as source_type  
             , s.label                                                                      as source_label
             , c.base_name ||'.'|| c.target                                                 as target_shape_id
             , t.shapetype                                                                  as target_type  
             , t.label                                                                      as target_label
             , CASE WHEN c.source = c.target THEN 'self precedence' ELSE c.shapetype END    as connection_type
...
def to_markdown_file(xml_path, shapes, connections):
    ...
    diagram_file_name = diagram_name.replace(" ", "%20")

    with open(md_filename, 'w') as md_file:
        md_file.write(f"# Overview for Diagram **{diagram_name}**:\n\n")
        
        md_file.write(f"![Diagram {diagram_name}](../png/{diagram_file_name}.png)\n")
